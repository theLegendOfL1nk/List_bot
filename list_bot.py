...
from discord.ext import commands # Needed for CommandGroup
import re
import time
import asyncio
import json
...
# Old prefix command constants removed as they are no longer needed.
...
# Initial data remains the same... (omitted for brevity)
INITIAL_DATA_LIST = []
# --- END CONFIGURATION ---
...
intents.guilds = True
intents.members = True # Essential for proper slash command context
...
SORT_CONFIGS = {
    # ... (Your SORT_CONFIGS dictionary remains the same) ...
    "sort_config_item": {
...
# --- DATA PERSISTENCE FUNCTIONS (Remain Unchanged) ---
def load_data_list():
    """Loads item list data AND channel state data from the JSON file."""
    # ... (Implementation remains the same) ...
    global data_list
    global channel_list_states
    # ... (Initial data setting/error handling remains the same) ...
...
class EphemeralListView(View):
    ...
    async def _update_ephemeral_message(self, interaction: discord.Interaction, new_sort_key: str):
        ...
        try:
            ...
            pass # Fails if message times out
...
# --- LIST FORMATTING AND UPDATE FUNCTIONS (Remain Unchanged) ---
...
async def on_ready():
    ...
    try:
        # NOTE: Using tree.sync() will update globally (up to 1 hour delay) or
        # per-guild if you pass a discord.Object(id=GUILD_ID).
        await tree.sync()
...
async def on_message(m: discord.Message):
    """
    Handles only the auto-update logic, as admin commands are now slash commands.
    """
...
async def main():
    ...
    try:
        ...
        # Gracefully stop the web server when the bot stops
        web_task.cancel()
